...
void WorldState::buildIndices() {
  ...
  for (size_t i = 0; i < venues.size(); ++i) {
    ...
    // global_venue_type_map is not touched here: in serial mode getVenueTypeId()
    // resolves every venue via getVenue(), and in MPI mode the loader's
    // buildGlobalVenueMaps has already filled it for every Venue in the world.
  }
...
void WorldState::dropGlobalVenueMaps() {
  ...
  std::cerr << "[HALO] freeing OTF maps: geo_unit_map_entries="
            ...
            << " (all-venue type_map kept, entries="
            << global_venue_type_map.size()
            << ")\n";
